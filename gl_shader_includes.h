...
static key_value src_includes[] = {
    ...
    {"punctual.glsl", R"( 
            ...
            vec3 baseColor, float ior)
        {
            ...
            pointToLight.y *= -1.0;  // TEMP correct inverted Y
            v.y *= -1.0;  // TEMP correct inverted Y
            n.y *= -1.0;  // TEMP correct inverted Y
    ...
    {"ibl.glsl", R"( 
        ...
        vec3 getDiffuseLight(vec3 n)
        {
            ...
            //vec4 textureSample = texture(u_LambertianEnvSampler, n);
        ...
        vec4 getSpecularSample(vec3 reflection, float lod)
        {
          ...
          //vec4 textureSample = textureLod(u_GGXEnvSampler, reflection, lod);
            textureSample.rgb *= u_EnvIntensity;
        ...
        vec4 getSheenSample(vec3 reflection, float lod)
        {
            ...
            //vec4 textureSample =  textureLod(u_CharlieEnvSampler, reflection, lod);
            textureSample.rgb *= u_EnvIntensity;
            return textureSample;
        }
        ...
        {
            float NdotV = clampedDot(n, v);
            //roughness = 0.85;//clamp(roughness, 0.0, 1.0);
            //roughness = (roughness + 2.0) / 3.0;
            float lod = roughness * float(u_MipCount - 1);

            v.y *= -1.0; //TEMP fix inverted Y
            n.y *= -1.0; //TEMP fix inverted Y
