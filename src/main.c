...
{
	LV_LOG_USER("scroll event");
	int32_t diff = lv_event_get_rotary_diff(e);
	lv_obj_t *viewer = lv_event_get_target_obj(e);

	lv_gltf_set_distance(viewer, lv_gltf_get_distance(viewer) + diff);
}
...
static void control_panel(lv_obj_t *parent, lv_gltf_model_t *model)
{
	...
	lv_obj_set_size(control_panel, CONTROL_PANEL_WIDTH, WINDOW_HEIGHT);
...
void lv_demo_gltf(const char *path)
{
	...
	lv_obj_add_event_cb(viewer, on_scroll, LV_EVENT_ROTARY, mouse_state);
...
int main(void)
{
	...
	/* GLTF setup */
	lv_demo_gltf("gltfs/webp_DiffuseTransmissionPlant.glb");
	/*lv_demo_gltf("gltfs/logo1.glb");*/
