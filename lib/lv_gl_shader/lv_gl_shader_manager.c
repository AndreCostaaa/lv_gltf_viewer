...
uint32_t select_shader(lv_gl_shader_manager_t * shader,
                       const char * shader_identifier,
                       lv_gl_shader_t * permutations, size_t permutations_len)
{
    ...
    if(hash == 1150100005) {
        LV_LOG_USER("#Permutations: %zu", permutations_len);
        LV_LOG_USER("Vertex %d shader %d \n%s",
                    string_ends_with(shader_identifier, ".vert"),
                    shader_map_key.id, shader_source);
    }
    else if((int32_t)hash == -1187161179) {
        LV_LOG_USER("#Permutations: %zu", permutations_len);
        LV_LOG_USER("Fragment %d shader %d \n%s",
                    !string_ends_with(shader_identifier, ".vert"),
                    shader_map_key.id, shader_source);
    }
    lv_free(shader_source);
...
lv_gl_shader_program_t * get_shader_program(lv_gl_shader_manager_t * manager,
                                            uint32_t fragment_shader_hash,
                                            uint32_t vertex_shader_hash)
{
    ...
    if(program_id == 7) {
        LV_LOG_USER("Vertex and Fragment hashes for program 7: %d %d",
                    vertex_shader_hash, fragment_shader_hash);
    }
...
void store_texture(lv_gl_shader_manager_t * manager, uint32_t texture_hash,
                   uint32_t texture_id)
{
...
GLuint get_texture(lv_gl_shader_manager_t * manager, uint32_t texture_hash)
{
...
void opengl_shader_init(lv_gl_shader_manager_t * shader)
{
                                                     ...
                                                     char * frag_src)
{
    ...
    shader->select_shader = &select_shader;
    shader->get_shader_program = &get_shader_program;
    shader->set_texture_cache_item = &store_texture;
    shader->getCachedTexture = &get_texture;
    ...
    opengl_shader_init(shader);
    return shader;
}

void destroy_environment(lv_gl_shader_manager_env_textures_t * _env)
{
    LV_LOG_INFO("Destroying shader env resources");
    const unsigned int d[3] = { _env->diffuse, _env->specular,
                                _env->sheen
                              };
