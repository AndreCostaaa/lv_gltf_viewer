...
lv_gltf_override_t * add_by_node(lv_gltf_data_t * gltf_data, fastgltf::Node * node, OverrideProp which_prop, uint32_t data_mask, bool read_only)
{
    if(node == nullptr) {
        return nullptr;
    }
    ...
    lv_gltf_override_t new_override;
    new_override.prop = which_prop;
    new_override.data_mask = data_mask;
    new_override.data1 =
    new_override.data2 =
    new_override.data3 = which_prop == OP_SCALE ? 1.f : 0.f;
    new_override.data4 = 0.f;
    new_override.read_only = read_only;
    new_override.dirty = true;
    new_override.next_override = nullptr; // Initialize next_override to null
    ...
    if(gltf_data->overrides.find(node) != gltf_data->overrides.end()) {
        ...
        lv_gltf_override_t * existing_override = gltf_data->overrides[node];

        // Traverse to the end of the linked list of overrides
        while(existing_override->next_override != nullptr) existing_override = existing_override->next_override;

        gltf_data->all_overrides.push_back(new_override);
        existing_override->next_override = &gltf_data->all_overrides[gltf_data->all_overrides.size() - 1];
        return existing_override->next_override;
    }
    else {
        // No existing override, insert the new one
        gltf_data->all_overrides.push_back(new_override);
        gltf_data->overrides[node] = &gltf_data->all_overrides[gltf_data->all_overrides.size() - 1];
        return gltf_data->overrides[node];
    }
    return nullptr;
}
