...
#include <animation.glsl>
    ...
    void main()
    {
        ...
        //mat4 normalMatrix =  temp_makeNormalMatrixFromViewProj( u_ViewProjectionMatrix * u_ModelMatrix );
        //mat4 normalMatrix =  temp_makeNormalMatrixFromViewProj( u_ModelMatrix );
        mat4 normalMatrix =  transpose(inverse(modelMatrix));
...
//u_TempKtxSampler
//uniform samplerCube u_TempKtxSampler;
...
#include <iridescence.glsl>
    ...
    temp_origBaseColor.rgb = getDiffuseLight(n);//vec3(f_diffuse);
    //temp_origBaseColor.rgb = vec3(f_diffuse);
    ...
    //temp_origBaseColor.rgb = f_specular_dielectric;//vec3(f_diffuse);
        ...
        //TEMP
        //temp_origBaseColor.rgb = vec3(clamp(f_diffuse + f_specular_dielectric, vec3(0.0), vec3(1.0)));
        //temp_origBaseColor.rgb = vec3(mix(f_diffuse, f_specular_dielectric, 0.1));//vec3(0.0), vec3(1.0)));

        //f_dielectric_brdf_ibl = f_diffuse;
    ...
    // TEMP testing KTX
    //vec4 __textureSample = texture(u_LambertianEnvSampler, n);
    //l_diffuse = __textureSample.rgb * baseColor.rgb;
    ...
    #endif
    //temp_origBaseColor.rgb = (g_finalColor).xyz;
    
    #ifdef HAS_NORMAL_VEC3
        //g_finalColor = vec4(v_Normal.x * 0.5 + 0.5, v_Normal.y* 0.5 + 0.5, v_Normal.z* 0.5 + 0.5, 1.0);
    #else
        //g_finalColor = vec4(1.0, 0.0, 0.0, 1.0);
    #endif
    //g_finalColor.rg = v_texcoord_0;
    //g_finalColor.b = 0.0;
    //g_finalColor.rgb = temp_origBaseColor.rgb;


    vec4 temp_mrSample = texture(u_MetallicRoughnessSampler, getMetallicRoughnessUV());
    ...
    // TEMP testing KTX
    //vec4 __textureSample = texture(u_TempKtxSampler, n);
    //textureSample.rgb
    //g_finalColor.rgb = __textureSample.rgb * 1.0;
